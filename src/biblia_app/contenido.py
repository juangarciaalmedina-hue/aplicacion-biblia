...
def construir_system_prompt(lang_code: str, mode: str = "study") -> str:
    ...
    target_language = language_names.get(lang_code, "español de España")

    if mode == "question":
        return (
            "You are a careful Christian Bible assistant. "
            f"You must answer only in {target_language}. Do not mix languages. "
            "Use natural native wording for the selected language. "
            "Do not translate proper nouns awkwardly. "
            "Do not invent Bible verses, quotations, historical facts, or references. "
            "If a detail is uncertain, say so briefly instead of guessing. "
            "Prefer clear, pastoral, readable Markdown."
        )

    return (
        "You are a careful Christian Bible study assistant. "
        f"You must write only in {target_language}. Do not mix languages. "
        "Keep the style natural and native for that language, not literal or translated-sounding. "
        "Respect the requested structure, length, and tone exactly. "
        "Do not invent Bible verses, quotations, historical facts, or references. "
        "If a detail is uncertain, avoid speculation and stay general. "
        "Use clean Markdown with short sections when useful."
    )
...
def mensaje_configuracion_ia(lang_code: str) -> str:
    if lang_code == "ca":
        return (
            "Error: falta configurar la IA.\n\n"
            "Perquè l'aplicació pugui generar contingut, has de crear un fitxer `.env` a la carpeta principal del projecte amb aquest contingut:\n\n"
            "```env\n"
            "OPENROUTER_API_KEY=la_teva_clau\n"
            "OPENROUTER_MODEL=openai/gpt-4o-mini\n"
            "OPENROUTER_FALLBACK_MODELS=google/gemini-2.5-flash-lite,deepseek/deepseek-chat\n"
            "OPENROUTER_TEMPERATURE=0.2\n"
            "OPENROUTER_TOP_P=0.9\n"
            "```\n\n"
            "Sense aquesta clau, la IA no pot respondre."
        )
    if lang_code == "fr":
        return (
            "Erreur : la configuration de l'IA est manquante.\n\n"
            "Pour que l'application puisse générer du contenu, tu dois créer un fichier `.env` dans le dossier principal du projet avec ce contenu :\n\n"
            "```env\n"
            ...
            "OPENROUTER_TOP_P=0.9\n"
            "```\n\n"
            "Sans cette clé, l'IA ne peut pas répondre."
        )
    if lang_code == "en":
        return (
            "Error: AI is not configured.\n\n"
            "For the app to generate content, you must create a `.env` file in the project's main folder with this content:\n\n"
            "```env\n"
            ...
            "OPENROUTER_TOP_P=0.9\n"
            "```\n\n"
            "Without this key, the AI cannot answer."
        )
    return (
        "Error: falta configurar la IA.\n\n"
        "Para que la aplicación pueda generar contenido, tienes que crear un archivo `.env` en la carpeta principal del proyecto con este contenido:\n\n"
        "```env\n"
        ...
        "OPENROUTER_TOP_P=0.9\n"
        "```\n\n"
        "Sin esa clave, la IA no puede responder."
    )
...
religiones_mundo = sorted({
    ...
    "Religiones africanas tradicionales: Sistemas religiosos autóctonos de África",
...
def pantalla_principal(page: ft.Page, idioma="es", on_volver=None, inicio="biblia", on_volver_inicio=None):
    ...
    plain_item_translations = {
        "ca": {
            ...
            "Conjunto de tradiciones religiosas originadas en la India": "Aplec de tradicions religioses originades a l'Índia",
            ...
            "Sistemas religiosos autóctonos de África": "Sistemes religiosos autòctons de l'Àfrica",
            ...
            "Barnabas": "Bernabe", "Boaz": "Booz", "Cain": "Caïn", "Cornelio": "Corneli", "Crispo": "Crisp",
            "Efrain": "Efraïm", "Elcanah": "Elcana", "Eleazar": "Eleazar", "Eli": "Eli", "Enoc": "Henoc",
            ...
            "Micaias": "Micàiès", "Nahum": "Nahum", "Natan": "Natan", "Natanael": "Natanael", "Nehemias": "Nehemies",
            ...
            "Ruben": "Rubèn", "Saul": "Saül", "Sem": "Sem", "Set": "Set", "Sila": "Siles",
            "Silas": "Silas", "Simon de Cirene": "Simó de Cirene", "Simeon": "Simeó", "Sosthenes": "Sostenes",
            "Tadeo": "Tadeu", "Tiquico": "Tiquic", "Tito Justo": "Titus Just", "Tobias": "Tobies",
            "Urias": "Uries", "Uziel": "Uziel", "Zabulon": "Zabuló", "Zaqueo": "Zaqueu", "Zebedeo": "Zebedeu",
            "Zorobabel": "Zorobabel",
        ...
        "fr": {
            ...
            "Sistemas religiosos autóctonos de África": "Systèmes religieux autochtones d'Afrique",
        ...
        "en": {
            ...
            "Sistemas religiosos autóctonos de África": "Indigenous religious systems of Africa",
    ...
    inicio_preferido = inicio if inicio in {"biblia", "filtros", "comportamiento", "incredulo", "cristianos"} else "biblia"
    ...
    dd_tipo = ft.Dropdown(
        ...
        options=[
            ...
            ft.dropdown.Option(key="Solo versiculos", text=only_verses),
            ft.dropdown.Option(key="Estudio informativo", text=ui["study_info"]),
    ...
    dd_tamano = ft.Dropdown(
        ...
        options=[
            ...
            ft.dropdown.Option(key="600", text="600"),
        ],
        value="Ninguno",
        expand=True,
        bgcolor=theme["field_bg"],
    ...
    def manejar_bloqueos(e=None):
        ...
        opciones_tipo = [
            ft.dropdown.Option(key="Ninguno", text=ui["no_selection"]),
            ft.dropdown.Option(key="Solo versiculos", text=only_verses),
            ft.dropdown.Option(key="Estudio informativo", text=etiqueta_estudio_informativo),
        ]
        if estudio_versiculos_disponible:
            opciones_tipo.append(ft.dropdown.Option(key="Estudio versiculos", text=ui["verse_study"]))
        ...
        contenedor_tipo.visible = paso_actual in {"study_type", "generate"}
    ...
    def contar_palabras(texto: str) -> int:
        ...
        return len(re.findall(r"\b[\wÁÉÍÓÚáéíóúÑñÜü]+\b", texto_limpio))
    ...
    def construir_prompt_estudio():
        ...
        instruccion_estilo_bonito = (
            ...
            "Puedes usar algunos símbolos o emoticonos cristianos con moderación si encajan bien, por ejemplo 🙏 📖 ✝️ 🕊️ 🐟 ✨. "
            ...
            (
                ...
                "Pots fer servir alguns símbols o emoticones cristians amb moderació si encaixen bé, per exemple 🙏 📖 ✝️ 🕊️ 🐟 ✨. "
                ...
                (
                    ...
                    "Tu peux utiliser quelques symboles ou émoticônes chrétiens avec modération s'ils s'intègrent bien, par exemple 🙏 📖 ✝️ 🕊️ 🐟 ✨. "
                    ...
                    "You may use a few Christian symbols or emojis in moderation when they fit naturally, for example 🙏 📖 ✝️ 🕊️ 🐟 ✨. "
    ...
    def ejecutar_consulta(e):
        ...
        result_md.value = generating_text
        ...
        def tarea():
            ...
            try:
                ...
                result_md.value = reforzar_respuesta_si_no_respeta_longitud(respuesta, prompt, "study")
            finally:
                pr.visible = False
                pr_comportamiento.visible = False
                pr_incredulo.visible = False
    ...
    def repetir_ultima_consulta(e):
        ...
        pr_incredulo.visible = False
        pr_cristianos.visible = False
        result_md.value = repeating_text
        ...
        def tarea():
            try:
                ...
                result_md.value = reforzar_respuesta_si_no_respeta_longitud(respuesta, ultimo_prompt_estudio, "study")
            finally:
                pr.visible = False
                pr_comportamiento.visible = False
                pr_incredulo.visible = False
    ...
    def preguntar_ia(e):
        ...
        contexto_resultado = result_md.value.strip()
        ...
        if lang_code == "es":
            prompt = (
                ...
                "Responde en Markdown, con apartados breves si ayudan. Cuida la presentación visual del texto: usa un título bonito y reverente y, si encaja, algunos símbolos cristianos con moderación como 🙏 📖 ✝️ 🕊️ 🐟. "
            ...
            prompt = (
                ...
                "Respon en Markdown, amb apartats breus si ajuden. Cuida la presentació visual del text: fes servir un títol bonic i reverent i, si encaixa, alguns símbols cristians amb moderació com 🙏 📖 ✝️ 🕊️ 🐟. "
            ...
            prompt = (
                ...
                "Réponds en Markdown, avec de courtes sections si cela aide. Soigne la présentation visuelle du texte : utilise un beau titre révérencieux et, si cela convient, quelques symboles chrétiens avec modération comme 🙏 📖 ✝️ 🕊️ 🐟. "
            ...
            prompt = (
                ...
                "Respond in Markdown, using short sections when helpful. Pay attention to the visual presentation of the text: use a beautiful reverent title and, when appropriate, a few Christian symbols in moderation such as 🙏 📖 ✝️ 🕊️ 🐟. "
        ...
        def tarea():
            try:
                ...
                result_md.value = reforzar_respuesta_si_no_respeta_longitud(respuesta, prompt, "question")
            finally:
                pr.visible = False
                pr_comportamiento.visible = False
                pr_incredulo.visible = False
    ...
    def actualizar_disposicion():
        ...
        vacio = not pr.visible and not pr_comportamiento.visible and not pr_incredulo.visible and not pr_cristianos.visible and not result_md.value.strip()
    ...
    page.add(
        ft.SafeArea(
            ft.Container(
                ...
                padding=1,
                border=ft.border.all(12, theme["primary"]),
                border_radius=26,
