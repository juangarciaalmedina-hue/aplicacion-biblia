...
def pantalla_principal(page: ft.Page, idioma="es", on_volver=None, inicio="biblia", on_volver_inicio=None):
    ...
    def construir_prompt_chat_suenos() -> str:
        ...
        prompts = {
            "es": (
                ...
                "Da siempre al menos dos posibles lecturas. Primero escribe literalmente 'La interpretación principal podría ser...' y explica la lectura que consideres más probable con lo contado. Después escribe literalmente 'Y otra posible interpretación sería...' y ofrece una alternativa prudente. No respondas con una sola interpretación. "
                ...
                "Responde como en un chat real, con frases cortas y naturales. "
                "Normalmente responde en 5 a 7 frases breves, sin Markdown. Usa siempre las frases 'La interpretación principal podría ser...' y 'Y otra posible interpretación sería...' para que las dos lecturas queden claras. "
    ...
    def asegurar_cierre_oracion_chat_consejero(texto: str, mensaje_usuario: str) -> str:
        ...
        respuesta = re.sub(
            r"(?i)[,;:\-]?\s*en el nombre de jes(?:u|Ãƒº|ú)s\.?\s*am(?:e|ÃƒÂ©|é)n\.?\s*$",
            "",
            respuesta,
        ).strip()
    ...
    def obtener_nombre_usuario_chat_consejero() -> str:
        patrones = (
            r"\bme llamo\s+([A-Za-zÁÃ‰ÃÃ“ÃšÃœÃ‘áéíóúüñ][A-Za-zÁÃ‰ÃÃ“ÃšÃœÃ‘áéíóúüñ'\-]{1,30})\b",
            r"\bsoy\s+([A-Za-zÁÃ‰ÃÃ“ÃšÃœÃ‘áéíóúüñ][A-Za-zÁÃ‰ÃÃ“ÃšÃœÃ‘áéíóúüñ'\-]{1,30})\b",
            r"\bmi nombre es\s+([A-Za-zÁÃ‰ÃÃ“ÃšÃœÃ‘áéíóúüñ][A-Za-zÁÃ‰ÃÃ“ÃšÃœÃ‘áéíóúüñ'\-]{1,30})\b",
        )
        for rol, mensaje, _ in reversed(historial_chat_consejero):
            if rol != "user":
                continue
    ...
    def recortar_solo_oracion_chat_consejero(texto: str) -> str:
        ...
        patron_cierre = re.search(
            r"(?i)\ben el nombre de jes(?:u|Ãƒº)s\.?\s*am(?:e|ÃƒÂ©)n\.?",
            respuesta,
        )
